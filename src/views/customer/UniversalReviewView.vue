...
const submitReview = async () => {
  ...
  try {
    ...
    if (reviewableType.value === "service") {
      ...
      formData.append('reviewable_id', reviewableId.value);
      formData.append('reviewable_type', reviewableType.value);
      if (route.params.merchantId) {
        formData.append('merchant_id', route.params.merchantId);
      }
      if (orderId.value) {
