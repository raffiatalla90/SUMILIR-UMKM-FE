<template>
  ...
  <div class="flex items-center gap-2">
    <button
      ...
      :class="isSameDay(selectedDate, d.date)
    ...
    <!-- Bottom bar -->
    <div
      class="fixed left-0 right-0 bottom-0 z-20 bg-white/95 backdrop-blur border-t border-gray-200 px-4 py-3"
    >
      <div class="max-w-screen-sm mx-auto flex items-center gap-4">
        <!-- Tombol chat: kotak oranye dengan icon Chat.png -->
        <button
          type="button"
          class="flex items-center justify-center w-11 h-11 rounded-xl bg-[#FFA30E]"
        >
        ...
        <router-link
          ...
          class="flex-1 py-3 rounded-full bg-[#FFA30E] hover:bg-[#e5920d] text-white font-semibold text-center"
        >
          Booking Sekarang
        </router-link>
      </div>
...
const dayName = (d) =>
  d.toLocaleDateString('id-ID',{weekday:'long'}).replace(/^\w/u,c=>c.toUpperCase())
...
const selectQuick = (d) => { selectedDate.value = new Date(d) }
...
const pkgList = computed(() => {
  return (jasa.value?.packages && jasa.value.packages.length)
    ? jasa.value.packages
    : [
        { id:1, name:'Paket 1', price:149999, image:'https://picsum.photos/seed/pk1/320/200' },
        { id:2, name:'Paket 2', price:244999, image:'https://picsum.photos/seed/pk2/320/200' },
        { id:3, name:'Paket 3', price:454500, image:'https://picsum.photos/seed/pk3/320/200' },
      ]
})
